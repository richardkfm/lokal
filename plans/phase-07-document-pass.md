...
2. **Locale** — request locale drives rulepack prose and dates; one date format;
   ICU plurals.
3. **Numbers** — the doubled euro tile; "735 von 180 insgesamt"; every phase
   named, including the empty ones.
4. **Typography and repetition** — a document type scale distinct from the app
   shell; §4's shared criteria stated once; §2 earning its place.
5. **Questionnaire** — nothing pre-filled; a review step that shows every
   answer; an error summary that moves focus.
...
- **Writing Impressum, Datenschutzerklärung or Barrierefreiheitserklärung
  text, or hosting it.** These are legal declarations about a specific operator:
  their name, address, data-protection officer, and their own conformance
  assessment. lokal cannot author them, and a placeholder is worse than an
  absence because a placeholder looks discharged — the same reason
  `LOKAL_EXPERT_*` renders nothing rather than an example contact. They ship as
  `LOKAL_LEGAL_*_URL` links to the pages the operator already publishes, rather
  than internal routes rendering operator-supplied markup: that was the other
  option, and it needs a content pipeline for three pages every Kommune and
  Stadtwerk in this audience already has. Unset means absent.
...
**One figure, not two.** The savings section rendered "Aktuell pro Jahr" and
...
**Every phase is named, including the empty ones.** A gap between "Phase 0" and
"Phase 2" is a question the document cannot answer, and the numbering is a
property of the rulepack the reader cannot see. Renumbering contiguously was the
obvious fix and hides the same fact more neatly; rendering the empty slot
explicitly — "Phase 1 — Schnelle Vorhaben: für diese Organisation
...
**The document gets its own type scale.** The report was `text-sm` and
...
keeps its scale; `.report-print` and the report view get a larger one.

The section number stays inline rather than hanging in the left margin, which
was the plan: the report container has no margin to hang into at narrow widths,
so it would have needed a breakpoint and a second layout to maintain. It is now
a text node followed by a real space instead of a flex child, which is what
actually fixed the accessible name reading "1Zusammenfassung" — a flex gap is
not a space, and that was the whole bug.
...
show one — were both already solved elsewhere.) The wizard pre-filled
...
the "Angaben prüfen" step, which showed six of roughly thirty-five answers while
saying "Der Plan wird ausschließlich hieraus berechnet", made it uncheckable.
